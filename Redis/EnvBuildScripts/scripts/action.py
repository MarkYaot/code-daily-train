...
def start_redis(redis_type):
    ...
    if "single" == redis_type:
        ...
        subprocess.run('nohup "%s/codis-server" %s/redis-proxy-9005.conf > %s/9005.log 2>&1 &' % (
            codis_path, conf_dst_path, log_path), shell=True)
        subprocess.run(
            'nohup "%s/codis-proxy" "--config=%s/proxy-1000.toml" "--dashboard=18080" "--log=%s/1000.log" '
            '"--log-level=INFO" "--ncpu=4" "--pidfile=/var/run/proxy-1000.pid" > "%s/1000.out" 2>&1 < /dev/null &' % (
                codis_path, conf_dst_path, log_path, log_path), shell=True)
        subprocess.run(
            'nohup "%s/codis-proxy" "--config=%s/proxy-1001.toml" "--dashboard=18080" "--log=%s/1001.log" '
            '"--log-level=INFO" "--ncpu=4" "--pidfile=/var/run/proxy-1001.pid" > "%s/1001.out" 2>&1 < /dev/null &' % (
                codis_path, conf_dst_path, log_path, log_path), shell=True)
        subprocess.run(
            'nohup "%s/codis-proxy" "--config=%s/proxy-1002.toml" "--dashboard=18080" "--log=%s/1002.log" '
            '"--log-level=INFO" "--ncpu=4" "--pidfile=/var/run/proxy-1002.pid" > "%s/1002.out" 2>&1 < /dev/null &' % (
                codis_path, conf_dst_path, log_path, log_path), shell=True)
        time.sleep(1)
        print("纳管Proxy节点")
